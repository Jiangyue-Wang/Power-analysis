...
write.csv(post_power,"results/02-power.csv",row.names = F)
...
for(i in 1:3){
  ...
  ggsave(paste0("results/02_woody_",post_power$Group[i],".png"), width = 6, height = 4)
  saveRDS(power_results,paste0("results/02_woody_",post_power$Group[i],".rds"))
  # flower second
  power_levels <- c()
  for(j in 1: length(sample_sizes_to_try)){
  ...
  ggplot(power_results, 
    ...
    scale_y_continuous(labels = scales::percent) + 
    labs(x = 'Sample Size', y = 'Power')
  ggsave(paste0("results/02_flowr_",post_power$Group[i],".png"), width = 6, height = 4)
  saveRDS(power_results,paste0("results/02_flowr_",post_power$Group[i],".rds"))
}